...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(167,139,250,0.08)_0%,_transparent_60%)]" />
      {Array.from({ length: 15 }).map((_, i) => (
        <div
          key={i}
          className="fixed rounded-full bg-purple-400/15 pointer-events-none"
          style={{
            width: ((i * 3 + 2) % 6) + "px",
            height: ((i * 3 + 2) % 6) + "px",
            left: ((i * 27 + 7) % 100) + "%",
      ...
      <div className="glass-card p-8 sm:p-10 w-full max-w-md relative">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-zinc-500 mt-1">Entre no SoundCircle</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-10 py-2.5 text-sm text-zinc-100 placeholder-zinc-600 focus:border-purple-500/40 focus:bg-white/10 transition-all"
              />
            </div>
          </div>
          <div>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-[#0b0b17] px-4 text-zinc-600">ou continue com</span>
          </div>
        </div>
